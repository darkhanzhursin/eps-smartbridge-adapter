...
import kz.ezdrav.eps_smartbridge_adapter.model.ws.eps.GetServicesByDate;
...
@Component
public class GetServicesByDateBuilder implements SoapMessageBuilder {

    private final String template = " is mandatory field";

    @Override
    public void buildBody(SOAPBody soapBody, Object data) throws SOAPException {
        GetServicesByDate request = (GetServicesByDate) data;
        SOAPElement element = soapBody.addChildElement(KaySoapConstants.GET_SERVICES_BY_DATE, KaySoapConstants.KAY);
    ...
    private void buildParams(SOAPElement element, ParamsDate params) throws SOAPException {
        ...
        var pageNo = Optional.of(params.getPageNo())
                .orElseThrow(() -> new FieldMandatoryException(KaySoapConstants.PAGE_NO + template));
        SOAPElement pageNoElement = paramsElement.addChildElement(KaySoapConstants.PAGE_NO, KaySoapConstants.KAY);
        pageNoElement.addTextNode(pageNo.toString());

        var pageSize = Optional.of(params.getPageSize())
                .orElseThrow(() -> new FieldMandatoryException(KaySoapConstants.PAGE_SIZE + template));
        SOAPElement pageSizeElement = pageNoElement.addChildElement(KaySoapConstants.PAGE_SIZE, KaySoapConstants.KAY);
        pageSizeElement.addTextNode(pageSize.toString());
